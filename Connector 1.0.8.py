...
def setup(datetime=datetime):
    ...
    if len(results) < 175:
        ...
        num = random.randint(175, 200)  # set to 5 for testing
        ...
        while count1 < num:
            if count2 < 5:  # test
                ...
                try:
                    ...
                    name_bool = False
                    try:
                        current_position = source.index('data":{"firstName":"', start_pos,
                                                        end_pos)
                    except ValueError:
                        name_bool = True
                    else:
                        start_pos = current_position + 20
                        try:
                ...
                area_list = ["new york", "new jersey", "toronto", "mississauga", "brampton", "scarborough",
                             ...
                             "ohio",
                             "washington dc", "washington, dc", "montreal", "washington d.c", "canada"]
                ...
                if not skill_fragment:
                    ...
                    sentence = "Looking at your profile, I couldn't help but notice your years of experience & your strong "\
                               + random.choice(interpersonal_skills_array) + " skills. "
                else:
                    k = skill_fragment.rfind(', ')
                    if skill_fragment.count(', ') >= 3:
                        skill_fragment = skill_fragment[:k] + ', & ' + skill_fragment[k + 1:]
                    elif skill_fragment.count(', ') == 2:
                        skill_fragment = skill_fragment[:-2]
                        skill_fragment = skill_fragment.replace(",", " &")
                    else:
                        skill_fragment = skill_fragment.replace(",", "")
                    sentence = "Looking at your profile, I couldn't help but notice that you've been endorsed for your strong "\
                ...
                if connect_bool:
                    ...
                    try:
                        ...
                        message_script = greeting + name + "! " + sentence + "I'm always looking to network with strong professionals. Ever thought about using your skills to diversify & develop your own business?"
                    ...
                    try:
                        WebDriverWait(driver, 15).until(
                            lambda driver: driver.find_element_by_xpath("//div[contains(@class,'mn-heathrow-org-toast')]"))
                    except TimeoutException:
                        if str(driver.current_url) != str(results[count1]):
                            pass
                        else:
                            count1 += 1
                            count2 += 1
                        ...
                        print(str(count3 + 1), end=' | ')
                        if round(end_time - start_time, 0) < 15:
                            print('Pausing ' + str(15 - (round(end_time - start_time, 0))) + ' seconds')
                            time.sleep(15 - (round(end_time - start_time, 0)))
                count1 += 1
                count3 += 1
                count2 = 0
                # time.sleep(600)
