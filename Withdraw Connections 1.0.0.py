...
try:
    os.system('tasklist | find /i "chromedriver.exe" && taskkill /im chromedriver.exe /F || echo chromedriver not running')
except Exception:
    print('...', end="")
finally:
    time.sleep(5)
...
def set_up():
    ...
    while True:
        if reit:
            try:
                past = WebDriverWait(driver, 60).until(lambda driver: driver.find_elements_by_xpath("//time[contains(@class, 'time-badge')]")).get_attribute('innerText')
                ...
                print(len(past))
                try:
                    withdraw = WebDriverWait(driver, 60).until(lambda driver: driver.find_elements_by_xpath("//button[contains(@data-control-name, 'single')]")).get_attribute('innerText')
                except TimeoutException:
                    ...
                    print(len(withdraw))
            reit = False
            elements = list(zip(past, withdraw))
        if (len(past) == 0) or (len(withdraw) == 0):
            break
        else:
            for e in elements:
                print(str(e))
